...
const unsigned int desiredRpm = 10;
float stepsPerRev = 360 / dl1;
// 1. convert RPM to revolutions per sec
const float rps = desiredRpm / 60;
// 2. calcuate total step per second
unsigned long stepsPerSec = stepsPerRev * rps;
// 3. calcuate pulse interval in ms
unsigned int pulseInterval = 150; // 1000000 * 1 / stepsPerSec;  // in micro sec.
unsigned int stepInterval = pulseInterval / 3;          // in us, ISR timer interrupt freq to be at least 100 time higher than the pulse rate.
...
void setup() {
  ...
  // Timer1.initialize(50);  // 3 times higher than the pulse rate.
  // Timer1.start();
...
void goTrajectory(float Jf[]) {
  ...
  dir1 = targetPos1 - curPos1 > 0.0;
  dir2 = targetPos2 - curPos2 > 0.0;
  dir3 = targetPos3 - curPos3 > 0.0;
  dir4 = targetPos4 - curPos4 > 0.0;
  dir5 = targetPos5 - curPos5 > 0.0;
  dir6 = targetPos6 - curPos6 > 0.0;
...
void stepMotors() {
  ...
  currentMillis = micros();  // Get the current time
  // Only step motors if enough time has passed
  if (currentMillis - lastStepTime >= pulseInterval) {
    // Joint 1
    ...
    lastStepTime = currentMillis;  //update last step time
  }
}
